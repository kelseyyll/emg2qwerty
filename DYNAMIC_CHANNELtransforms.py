...
class ToTensor:
    ...
    Args:
        ...
        stack_dim (int): The new dimension to insert while stacking
            ``fields``. (default: 1)
        num_channels (int): The number of EMG channels to extract (e.g., 8 instead of 16).
    """

    fields: Sequence[str] = ("emg_left", "emg_right")
    stack_dim: int = 1
    num_channels: int = 8  # Ensure we extract only 8 channels

    def __call__(self, data: np.ndarray) -> torch.Tensor:

        # Should be structured array
        if not hasattr(data, "dtype") or not data.dtype.names:
            print(f"ERROR: {type(data)} of {data.shape}")
            raise TypeError("not structured NumPy array")

        tensors = []
    
        for f in self.fields:
            try:
                arr = data[f][:, :self.num_channels]  # Extract only up to num_channels
                tensors.append(torch.as_tensor(arr))
            except Exception as e:
                raise

        return torch.stack(tensors, dim=self.stack_dim)  # Stack along correct dimension
