...
class EMGSessionData:
    ...
    def __getitem__(self, key: slice | str) -> np.ndarray:
        ...
        # Convert to np array
        data = {f: np.array(self.timeseries[key][f]) for f in self.timeseries.dtype.fields}

        if EMGSessionData.TIMESTAMPS not in data:
            data[EMGSessionData.TIMESTAMPS] = np.array(self.timeseries[key][EMGSessionData.TIMESTAMPS])
        data[EMGSessionData.EMG_LEFT] = data[EMGSessionData.EMG_LEFT][:, :8]
        data[EMGSessionData.EMG_RIGHT] = data[EMGSessionData.EMG_RIGHT][:, :8]
...
class WindowedEMGDataset(torch.utils.data.Dataset):
    ...
    hdf5_path: Path
    num_channels: int # CHANGE CHANNELS DYNAMICALLY
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Fetch window with contextual padding
        ...
        # Convert correctly to np array
        if isinstance(window, dict):
        
            if EMGSessionData.EMG_LEFT not in window or EMGSessionData.EMG_RIGHT not in window:
                raise KeyError("Missing either EMG_LEFT or EMG_RIGHT key")

            dtypes = [(EMGSessionData.EMG_LEFT, '<f4', (self.num_channels,)), 
                      (EMGSessionData.EMG_RIGHT, '<f4', (self.num_channels,)),
                      (EMGSessionData.TIMESTAMPS, '<f8', (1,))] 

            structured_array = np.zeros(len(window[EMGSessionData.EMG_LEFT]), dtype=dtypes)
            structured_array[EMGSessionData.EMG_LEFT] = window[EMGSessionData.EMG_LEFT]
            structured_array[EMGSessionData.EMG_RIGHT] = window[EMGSessionData.EMG_RIGHT]
            structured_array[EMGSessionData.TIMESTAMPS] = window[EMGSessionData.TIMESTAMPS].reshape(-1, 1)

            window = structured_array  

        # Extracting channels
        try:
            emg_left = window[EMGSessionData.EMG_LEFT][:, :self.num_channels]
            emg_right = window[EMGSessionData.EMG_RIGHT][:, :self.num_channels]
        except Exception as e:
            print(f"error for channel {e}")
            raise

        emg = np.stack([emg_left, emg_right], axis=1)  # Shape: (T, 2, num_channels)
        emg = self.transform(window) 
        assert torch.is_tensor(emg), "Transform output is not a tensor!"

        if "time" in window.dtype.names:
            timestamps = window["time"]
        else:
            raise ValueError(f"Missing `time` field in structured array! Available fields: {window.dtype.names}")

        start_idx = max(offset - window_start, 0)  
        end_idx = min((offset + self.window_length - 1) - window_start, len(timestamps) - 1)  
        start_t = timestamps[start_idx]
        end_t = timestamps[end_idx]
        
        # Correct labels and assigning
        gt_labels = self.session.ground_truth(start_t, end_t).labels  
        if len(gt_labels) == 0:
            labels = torch.zeros(1, dtype=torch.int32)  # then empty tensor
        else:
